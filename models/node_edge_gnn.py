...
class NodeEdgeConv(MessagePassing):
    ...
    def __init__(self, node_in_channels: int, node_out_channels: int, edge_in_channels: int, edge_out_channels: int,
        ...
        node_update_input_size = (node_in_channels + hidden_size * 2)
        ...
        edge_update_input_size = (edge_in_channels + hidden_size * 2)
    ...
    def message(self, w_x_j: Tensor, edge_attr: Tensor, alpha: Tensor):
        edge_attr = self.edge_mlp(edge_attr)
        return alpha * torch.cat([edge_attr, w_x_j], dim=-1)
