...
class NodeEdgeAttnConv(MessagePassing):
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_attr: Tensor):
        ...
        undir_edge_index, undir_edge_attr = self._to_undirected(edge_index, edge_attr)

        alpha = self.compute_attention(undir_edge_index, x=x, edge_attr=undir_edge_attr)

        out, msg = self.propagate(undir_edge_index, x=x, alpha=alpha, edge_attr=undir_edge_attr)
    ...
    def _to_undirected(self, edge_index: Tensor, edge_attr: Tensor) -> Tuple[Tensor, Tensor]:
        '''Convert to undirected edges such that first half are the original edges and second half are the flipped edges'''
        row, col = edge_index[0], edge_index[1]
        row, col = torch.cat([row, col], axis=0), torch.cat([col, row], axis=0)
        edge_index = torch.stack([row, col], axis=0)

        edge_attr = torch.cat([edge_attr, edge_attr], axis=0)
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_attr: Tensor) -> Tensor:
        ...
        # msg[edge_attr.shape[0]:] *= -1  # Flip the second half of the messages for undirected edges
        return msg

    def update(self, aggr_out: Tensor) -> Tensor:
        return self.node_update_mlp(aggr_out)

    def edge_update(self, msg_to: Tensor, msg_from: Tensor, out_i: Tensor, out_j: Tensor, index: Tensor, ptr, dim_size: Optional[int]) -> Tensor:
