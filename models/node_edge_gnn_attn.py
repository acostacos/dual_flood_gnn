...
class NodeEdgeGNNAttn(BaseModel):
    ...
    def _make_gnn(self, input_size: int, output_size: int, input_edge_size: int, output_edge_size: int,
                  hidden_size: int = None, num_layers: int = 1, activation: str = None, **conv_kwargs) -> Module:
        if num_layers == 1:
            return NodeEdgeLayer(input_size, output_size, input_edge_size, output_edge_size, activation, **conv_kwargs)

        hidden_size = hidden_size if hidden_size is not None else (input_size * 2)

        layers = []
        layers.append(
            (NodeEdgeLayer(input_size, hidden_size, input_edge_size, output_edge_size, activation, **conv_kwargs),
             'x, edge_index, edge_attr -> x, edge_attr')
        ) # Input Layer

        for _ in range(num_layers-2):
            layers.append(
                (NodeEdgeLayer(hidden_size, hidden_size, input_edge_size, output_edge_size, activation, **conv_kwargs),
                 'x, edge_index, edge_attr -> x, edge_attr')
            ) # Hidden Layers
...
class NodeEdgeLayer(Module):
    def __init__(self,
                 ...
                 **conv_kwargs):
        super().__init__()
        self.conv = NodeEdgeConv(in_features, out_features, in_edge_features, out_edge_features, **conv_kwargs)
...
class NodeEdgeConv(MessagePassing):
    ...
    def __init__(self,
        ...
        self.lin_edge = Linear(edge_in_features, out_features, bias=False)
        self.node_attn = Linear((out_features * 3), 1, bias=False)

        self.msg_lin = Linear((out_features * 2), edge_out_features, bias=False)

        node_update_input_size = (out_features * 2)
        self.node_update_mlp = Linear(node_update_input_size, out_features, bias=False,)
        ...
        self.edge_update_mlp = Linear(edge_update_input_size, edge_out_features, bias=False)

        if residual:
            self.residual = Linear(in_features, out_features, bias=False)
            self.residual_edge = Linear(edge_in_features, out_features, bias=False)

        if bias:
            self.bias = Parameter(torch.empty(out_features))
            self.bias_edge = Parameter(torch.empty(out_features))
            zeros(self.bias)
