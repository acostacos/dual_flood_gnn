...
class NodeGNNAttn(BaseModel):
    ...
    def _make_gnn(self, input_size: int, output_size: int, input_edge_size: int, output_edge_size: int,
        ...
        if gnn_layers == 1:
            return NodeAttnConv(input_size, output_size, input_edge_size, output_edge_size, **conv_kwargs)

        hidden_size = hidden_size if hidden_size is not None else (input_size * 2)

        layers = []
        layers.append(
            (NodeAttnConv(input_size, hidden_size, input_edge_size, hidden_size, **conv_kwargs),
             'x, edge_index, edge_attr -> x')
        ) # Input Layer

        for _ in range(gnn_layers-2):
            layers.append(
                (NodeAttnConv(hidden_size, hidden_size, hidden_size, hidden_size, **conv_kwargs),
                 'x, edge_index, edge_attr -> x')
            ) # Hidden Layers

        layers.append(
            (NodeAttnConv(hidden_size, output_size, hidden_size, output_edge_size, **conv_kwargs),
             'x, edge_index, edge_attr -> x')
        ) # Output Layer
...
class NodeAttnConv(MessagePassing):
